...
BASE_DIR = "B:/code projects/spotify display/"
...
def connect_serial(port = "COM9", baud = 115200, retries = 20, delay = 1):
    for i in range(retries):
        try:
            ...
            print(f"waiting for port | {i+1}/{retries}")
            time.sleep(delay)
...
def get_activity(token, refresh):
    ...
    if len(data.get("item").get("album").get("images")) > 0:
        ...
        cover = "B:/code projects/spotify display/assets/man.jpg" # placeholder
        ...
        if last.get("id") == id: # still listening to the same song
            ...
            if cooldown_counter > 2:
                ...
                if "https" in cover: # fetch link if link is there
                    ...
                    image = Image.open("B:/code projects/spotify display/assets/man.jpg")
