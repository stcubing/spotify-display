...
<figure>
    ...
    <figcaption>i love imaginal disk</figcaption>
</figure>


## process

This was a very foreign project to me so it really wass just a lot of messing around with random stuff until something happened. I already had a little bit of prior experience with the Spotify API (matchify :v) so that part wasn't as difficult, but trying to send over that information to the ESP-32 was confusing. After some reading, I eventually ended up sending some test messages over a serial port which (after minor troublehsooting) ended up working incredibly well. 
...
I also ended up buing a Bambu Lab A1 Mini during this project just for the housing. Initially, I wanted to use my friend's printer (with compensation!), but in retrospect, I'm glad I didn't torture him with the number of terrible versions I printed.
...
<figure style="display: flex; width: 100%; align-items: center; gap: 10px">
    ...
    <figcaption>almost unintelligible first concepts, some drawings from trying to figure out the housing</figcaption>
</figure>
