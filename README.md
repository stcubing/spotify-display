...
This was a very foreign project to me, so it really was just a lot of messing around with random stuff until something happened that nudged me in the right direction. I already had a bit of prior experience with the Spotify API when developing another project of mine (matchify :v), so getting the information wasn't as difficult, but trying to send that over to the ESP-32 was confusing. After some research, I ended up deciding on sending JSON messages over a serial port to be received and decoded by the ESP-32, which (after minor troubleshooting) ended up working incredibly well. 

This success was extremely short-lived as I realised how insanely slow the serial was when dealing with images (or binary representations of them). Such relatively large data slowed down the messages and consequently, screen updates, to a crawl, and it was probably the biggest hitch in the process of this project due to how directionless I was when trying to tackle this issue. In the end, I realised that simply sending all that image stuff in a separate serial message would fix everything. I also anticipated that handling and converting the image would be difficult, but thanks to the Pillow library, the dithering was incredibly trivial.
